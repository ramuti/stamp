...
function initUser(){
  ...
  function updateCardTitle(){ cardTitle.textContent = userName ? `${userName}のスタンプカード` : "スタンプカード"; }
  ...
  function renderUserCard(card){
    ...
    container.style.background=card.bg || "#fff0f5"; // ← 背景色反映
    ...
    btn.addEventListener("click",()=>{
      ...
      const used=userStampHistory.some(s=>s.cardId===card.id&&s.keyword===word);
      if(used){alert("もう押してあります"); return;}
      let nextSlot=0;
...
function initAdmin(){
  ...
  function refreshCardListUI(){
    ...
    cards.forEach(c=>{
      ...
      left.style.flex="1";
      left.innerText=`${c.name} | パス:${c.addPass} | 枠:${c.slots} | 色:${c.bg||"#fff0f5"}`;
      li.appendChild(left);
  ...
  function renderPreview(card){
    ...
    d.className="card";
    d.style.background=card.bg || "#fff0f5"; // ← 背景色反映
    const h=document.createElement("h3");
    h.textContent=`${card.name}（プレビュー）`;
    d.appendChild(h);
  ...
  createCardBtn.addEventListener("click",()=>{
    ...
    const bg=(cardBG.value||"#fff0f5").trim();
    ...
    const newCard={id:Date.now(),name,slots,addPass:pass,bg,bg,notifyMsg:"",maxNotifyMsg:"",stampImg:""};
    cards.push(newCard);
    saveAll();
    refreshCardListUI();
    renderPreview(newCard);
    cardName.value=""; cardSlots.value=5; addPass.value=""; cardBG.value="#fff0f5";
  });
