...
function initUser(){
  ...
  function renderUserCard(card){
    ...
    const serial=document.createElement("div"); serial.className="serial"; serial.textContent=genSerialForUser();
...
function initAdmin(){
  ...
  function refreshCardListUI(){
    ...
    cards.forEach(c=>{
      ...
      del.addEventListener("click",()=>{
        ...
        saveAll(); refreshCardListUI(); refreshKeywordSelect(); 
      });
  ...
  function refreshKeywordList(){
    ...
    keywords.forEach(k=>{
      const li=document.createElement("li"); li.textContent=`カード:${k.cardId} | ${k.word} | active:${k.active}`;
      const del=document.createElement("button"); del.textContent="削除"; del.style.background="#999";
      del.addEventListener("click",()=>{
        if(!confirm("削除しますか？")) return;
  ...
  createCardBtn.addEventListener("click",()=>{
    ...
    const newCard={id:Date.now().toString(),name:name,slots:slots,notifyMsg:notify,maxNotifyMsg:maxNotify,addPass:pass,bg:bg,stamp:stamp};
    cards.push(newCard); saveAll();
    refreshCardListUI(); refreshKeywordSelect();
    previewArea.appendChild(renderPreviewCard(newCard));
  });
  ...
  addKeywordBtn.addEventListener("click",()=>{
    ...
    if(!cid||!word){ alert("カードと合言葉を設定してください"); return; }
    keywords.push({cardId:cid,word:word,active:true}); saveJSON(LS_KEYS.keywords,keywords); refreshKeywordList();
  });

  addUpdateBtn.addEventListener("click",()=>{
    const text=updateInput.value.trim(); if(!text) return;
    updates.push(text); saveJSON(LS_KEYS.updates,updates); refreshUpdates(); updateInput.value="";
  });
