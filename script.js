...
function saveAll(){
  try{
    ...
    saveJSON(LS_KEYS.userAddedCards, mergeUniqueArray(loadJSON(LS_KEYS.userAddedCards,[]), userAddedCards));
    saveJSON(LS_KEYS.userStampHistory, mergeStampHistories(loadJSON(LS_KEYS.userStampHistory,[]), userStampHistory));
    saveJSON(LS_KEYS.userCardSerials, mergeUserCardSerials(loadJSON(LS_KEYS.userCardSerials,{}), userCardSerials));
...
function initUser(){
  ...
  setNameBtn.addEventListener("click",()=>{
    ...
    userName=val; saveAll(); renderUserID(); updateUserCardsTitle(); renderUserCards();
  });

  function renderUserID(){
    let el=document.getElementById("userIDDisplay");
    if(!el){ el=document.createElement("div"); el.id="userIDDisplay"; el.style.position="fixed"; el.style.right="8px"; el.style.bottom="8px"; el.style.fontSize="0.75em"; el.style.color="#999"; document.body.appendChild(el);}
  ...
  addCardBtn.addEventListener("click",()=>{
    ...
    saveAll(); addCardPassInput.value=""; renderUserCards(); renderStampHistory();
  });
  ...
  function renderUserCards(){
    ...
    userAddedCards.forEach(cid=>{
      ...
      div.className="card"; div.style.background=c.bg||"#fff0f5"; div.style.position="relative"; div.style.padding="8px"; div.style.marginBottom="16px";
      ...
      const stampBtn=document.createElement("button"); stampBtn.textContent="スタンプ押す"; stampBtn.style.display="block"; stampBtn.style.marginTop="8px";
      stampBtn.addEventListener("click",()=>{
        const inputWord=prompt("スタンプ合言葉を入力してください");
        if(!inputWord)return;
        const matchedKeyword=(keywords.find(k=>k.cardId===cid && k.word===inputWord && k.enabled))|| (cards.find(cc=>cc.id===cid && cc.addPass===inputWord));
        if(!matchedKeyword) return alert("合言葉が違います");
        if(userStampHistory.some(s=>s.cardId===cid && s.word===inputWord)) return alert("この合言葉は既に使われました");

        const stampedCount=userStampHistory.filter(s=>s.cardId===cid).length;
        if(stampedCount>=c.slots){ if(c.maxNotifyMsg) alert(c.maxNotifyMsg); return alert("もう押せません"); }

        userStampHistory.push({ cardId:cid, slot:stampedCount, word:inputWord, datetime:new Date().toISOString() });
      ...
      const serialDiv=document.createElement("div"); serialDiv.textContent=serial; serialDiv.style.position="absolute"; serialDiv.style.right="4px"; serialDiv.style.bottom="4px"; serialDiv.style.fontSize="0.7em"; serialDiv.style.color="#666";
  ...
  function renderStampHistory(){
    ...
    userStampHistory.slice().reverse().forEach(s=>{
      const card=cards.find(c=>c.id===s.cardId); if(!card)return;
      const li=document.createElement("li"); li.textContent=`${card.name} スロット${s.slot+1} ${new Date(s.datetime).toLocaleString()}`;
  ...
  function renderUpdates(){
    ...
    updates.forEach(u=>{ const li=document.createElement("li"); li.textContent=`${u.date} ${u.msg}`; updateLogsList.appendChild(li); });
  }

  renderUserCards(); renderStampHistory(); renderUpdates();
}
...
function initAdmin(){
  ...
  function renderAdminCards(){
    adminCardsList.innerHTML=""; keywordCardSelect.innerHTML="";
    cards.forEach(c=>{
      ...
      delBtn.addEventListener("click",()=>{
        ...
        for(const uname in userCardSerials){ if(userCardSerials[uname] && userCardSerials[uname][c.id]) delete userCardSerials[uname][c.id]; }
        userStampHistory=userStampHistory.filter(s=>s.cardId!==c.id); saveAll(); renderAdminCards();
      });
  ...
  function renderKeywords(){
    ...
    keywords.forEach((k,idx)=>{
      ...
      li.innerHTML=`${cName} : ${k.word} : 状態:${k.enabled?"有効":"無効"}`;
      ...
      const delBtn=document.createElement("button"); delBtn.textContent="消去"; delBtn.addEventListener("click",()=>{ keywords.splice(idx,1); saveAll(); renderKeywords(); });
  ...
  function renderUpdates(){
    ...
    updates.forEach((u,idx)=>{
      ...
      const delBtn=document.createElement("button"); delBtn.textContent="削除"; delBtn.addEventListener("click",()=>{ updates.splice(idx,1); saveAll(); renderUpdates(); });
      li.appendChild(delBtn); adminUpdateLogs.appendChild(li);
    });
  }
  ...
  previewCardBtn.addEventListener("click",()=>{
    ...
    previewArea.innerHTML=`<div class="card" style="background:${c.bg}">${c.name}<br>${Array.from({length:c.slots}).map(_=>`<span class="stamp-slot"></span>`).join("")}</div>`;
  });
...
function addCopyButton(){
  ...
  btn.addEventListener("click",()=>{
    if(typeof generateUpdateData==="function"){
      navigator.clipboard.writeText(generateUpdateData()).then(()=>alert("コピーしました！\nupdateDataFull.jsに上書きしてください")).catch(err=>alert("コピー失敗:"+err));
