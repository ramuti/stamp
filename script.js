...
let userSerials = loadJSON(LS_KEYS.userSerials, {}); // ユーザごとのカードID→シリアル
...
function saveAll() {
  try {
    ...
    saveJSON(LS_KEYS.userSerials, userSerials);
...
function initUser(){
  ...
  addCardBtn.addEventListener("click",()=>{
    ...
    if(!userAddedCards.includes(card.id)){
      ...
      // ユーザごとのシリアル番号を生成（固定）
      if(!userSerials[userName]) userSerials[userName]={};
      userSerials[userName][card.id]=Object.keys(userSerials).reduce((max,user)=>{
        const v=userSerials[user]?userSerials[user][card.id]||0:0; return Math.max(max,v);
      },0)+1;
      saveAll();
      renderUserCards();
      addCardPass.value="";
  ...
  function renderUserCard(card){
    ...
    serial.textContent = userSerials[userName]&&userSerials[userName][card.id]?userSerials[userName][card.id]:"";
...
function initAdmin(){
  ...
  function renderKeywords(){
    ...
    keywords.forEach((k,i)=>{
      ...
      span.textContent=`[${k.cardId}] ${k.word}`;
      li.appendChild(span);
  ...
  addUpdateBtn.addEventListener("click",()=>{
    ...
    if(!text) return;
    updates.push({text,date:new Date().toISOString()});
    saveJSON(LS_KEYS.updates,updates);
    renderUpdateLogs();
    updateInput.value="";
  });
