...
function saveAll(){
  ...
  }catch(e){alert("データ保存失敗");console.error(e);}
}
...
function initUser(){
  ...
  addCardBtn.addEventListener("click",()=>{
    ...
    if(!userAddedCards.includes(card.id)){ userAddedCards.push(card.id); saveAll(); renderUserCards(); addCardPass.value=""; }
  ...
  function renderUserCard(card){
    ...
    for(let i=0;i<card.slots;i++){ const slot=document.createElement("div"); slot.className="stamp-slot"; if(userStampHistory.some(s=>s.cardId===card.id&&s.slot===i)) slot.classList.add("stamp-filled"); grid.appendChild(slot);}
    container.appendChild(grid);

    const serial=document.createElement("div"); serial.className="serial"; serial.textContent=genSerialForUser(); container.appendChild(serial);
    ...
    btn.addEventListener("click",()=>{
      ...
      saveAll(); renderUserCards(); updateHistory(); alert(card.notifyMsg||"スタンプを押しました！");
    });
  ...
  function renderUserCards(){
    ...
    saveAll();
    userAddedCards.forEach(id=>{ const card=cards.find(c=>c.id===id); if(card) userCards.appendChild(renderUserCard(card)); });
  }
  ...
  function updateHistory(){
    ...
    [...userStampHistory].reverse().forEach(h=>{
      ...
      const li=document.createElement("li"); li.textContent=`${card.name} — ${h.date}`; li.style.color=userUIColors.text;
...
function initAdmin(){
  ...
  function renderAdminCards(){
    ...
    cards.forEach(card=>{
      ...
      const info=document.createElement("div"); info.className="info"; info.textContent=`${card.name} [枠:${card.slots}]`; li.appendChild(info);
      ...
      btns.appendChild(delBtn); li.appendChild(btns); adminCards.appendChild(li);
  ...
  addKeywordBtn.addEventListener("click",()=>{
    ...
    keywords.push({cardId,word,active:true}); saveAll(); renderKeywordList(); keywordInput.value="";
  });
